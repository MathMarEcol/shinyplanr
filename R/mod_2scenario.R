    ...
    shiny::sidebarLayout(
      ...
      shiny::mainPanel(
        ...
        tabsetPanel(id = ns("tabs"),# type = "pills",
                    ...
                    tabPanel("Climate Resilience", value = 6,
                             shiny::fixedPanel(style="z-index:100", # To force the button above all plots.
...
## Binary Solution Plot ----------------------------------------------------
    ...
    observeEvent({input$tabs == 1}, {
      ...
      output$txt_soln <- shiny::renderText({
        paste("This plot shows the optimal planning scenario for the study area
...
## Cost Plot -------------------------------------------------------------
    ...
    observeEvent({input$tabs == 3}, {

      costPlotData <- shiny::reactive({

        spatialplanr::splnr_plot_costOverlay(selectedData(), Cost = NA, Cost_name = input$costid,
                                                   legendTitle = "Cost",
      ...
      output$hdr_cost <- shiny::renderText({"Rational Use Overlaid with Selection"}) %>%
        shiny::bindEvent(input$analyse)
...
## Climate Resilience Plot -------------------------------------------------
    ...
    observeEvent({input$tabs == 6}, {
      ...
      output$hdr_clim <- shiny::renderText({if (input$checkClimsmart == TRUE){paste("Climate Resilience")}}) %>%
        shiny::bindEvent(input$analyse)
